...
### Scientific Question
When examining dog breeds (canis lupus familiaris), will breeds of a similar size (e.g. Cocker Spaniel, English Cocker Spaniel) have more related genes (IGF1, IGSF1, LCORL, and SMAD2) and SNP's surrounding longevity than breeds of a different size (e.g.Doberman Pinscher,Miniature Pinscher)? Will these differences results in expression changes between breeds of different sizes?
...
### Gene analysis:
- Multiple Sequence Alignment-->Clustering: multiple sequence alignment (MSA) was performed on the genes. These alignments were converted into a data format feasible to cacluate a distance matrix from (and thus clustering) msaConvert(). Clustering was performed on this processed data and then visualized as a dendrogram. 
