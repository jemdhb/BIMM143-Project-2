...
```
The differences sequence-wise seem to be minor when looking at such a narrow scope and small subsection of dogs. Thankfully there is also expression data for the genes studied which covers more dog-breeds, depicted below
...
p<-ggplot(data = expression, mapping = aes_string(x="norm_exp",y="weight_kg" ,col= "size"))+geom_point(size=4,alpha=0.6)+ labs(y="Weight of Dog (kg)", x = "Expression of IGF1")+ggtitle("IGF1 Expression by Dog Weight (grouped by dog size)")
...
p<-ggplot(data = expression, mapping = aes_string(x="norm_exp",y="weight_kg" ,col= "size"))+geom_point(size=4,alpha=0.6)+ labs(y="Weight of Dog (kg)", x = "Expression of LCORL")+ggtitle("LCORL Expression by Dog Weight (grouped by dog size)")
p+scale_color_manual(values=myColors)
```
...
p<-ggplot(data = expression, mapping = aes_string(x="norm_exp",y="weight_kg" ,col= "size"))+geom_point(size=4,alpha=0.6)+ labs(y="Weight of Dog (kg)", x = "Expression of SMAD2")+ggtitle("SMAD2 Expression by Dog Weight (grouped by dog size)")
p+scale_color_manual(values=myColors)
```
